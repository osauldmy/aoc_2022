...
def first(play: str) -> int:
    opponent, you = map(int, play.split(" "))
...
def second(play: str) -> int:
    opponent, outcome = map(int, play.split(" "))
    if outcome == 1:  # lose
...
plays = data.translate(data.maketrans("AXBYCZ", "112233")).strip("\n").split("\n")
